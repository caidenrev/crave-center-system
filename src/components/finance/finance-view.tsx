...
export function FinanceView({ incomes, title }: { incomes: IncomeItem[], title?: string }) {
  ...
  const headerTitle = title || t("incomeHistoryTitle")
    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 w-full overflow-hidden">
      <div className="p-6 md:p-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-lg md:text-xl font-bold uppercase text-slate-900 dark:text-white tracking-wider">{headerTitle}</h2>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-t border-slate-100 dark:border-slate-800 text-slate-400 dark:text-slate-500 text-[11px] uppercase tracking-wider font-semibold">
              ...
              <th className="py-4 px-4 whitespace-nowrap">{t("colDescription")}</th>
              ...
              return (
                <tr key={item?.id || `empty-${idx}`} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                  <td className="py-4 px-6 md:px-8 text-slate-500 dark:text-slate-400 whitespace-nowrap" suppressHydrationWarning>
                    {isDummy ? <span className="opacity-0">{t("empty")}</span> : new Date(item.date).toLocaleDateString(locale, {
        ...
        {!hasIncomes && (
          <div className="flex flex-col items-center justify-center py-16 text-slate-400">
            ...
            <p className="text-sm font-medium">{t("noIncomeHistory")}</p>
          </div>
        )}
      </div>
    </div>
