...
export function WorkerSidebar({
  ...
  const bottomItems = [
    { name: t("settings"), href: `/${locale}/worker/settings`, icon: Settings },
  ...
  const TopBrandHeader = () => (
    <div className="h-16 flex items-center justify-between px-4 mt-2">
      <Link href={`/${locale}/worker`} className="flex items-center gap-3 w-full p-2 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <span className="text-xs text-slate-500 font-medium">Worker Workspace</span>
        </div>
      ...
      <div className="px-1 mb-2 mt-auto relative">
        ...
        >
          <div className="flex items-center gap-3 overflow-hidden">
            <img 
              src={user?.image || getDefaultAvatar(user?.name || user?.email || "Worker")} 
              alt="Avatar" 
              className="w-8 h-8 rounded-xl object-cover shrink-0 border border-slate-100 dark:border-slate-700" 
            />
            <div className="flex flex-col min-w-0">
              <span className="text-sm font-semibold text-slate-900 dark:text-white truncate leading-tight">{user?.name || "Worker"}</span>
              <span className="text-[10px] text-slate-500 truncate">{user?.email || "worker@crave.com"}</span>
            </div>
          </div>
